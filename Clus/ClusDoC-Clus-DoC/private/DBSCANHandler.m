...
% varargout =Variable-length output argument list
    ...
    varargout{2} = [];
    varargout{3} = [];
    varargout{4} = struct; % 应该是指代result
...
try 
    ...
    %   Data = Zen format
        ...
        if display1 || ~printOutFig
            ...
            sprintf("plot")
            fig1
            ax1 = axes('parent',fig1); % 指定fig1作为ax1的父对象
            set(ax1, 'NextPlot', 'add');
            plot(ax1, Data(:,1), Data(:,2), 'Marker', '.', 'MarkerSize', 5, 'LineStyle', 'none', 'color', rgb(127, 140, 141)); 
            %text(ax1, Data(:,1), Data(:,2), "1")
            axis image tight  % 动态调整坐标轴，使其紧凑
        end
    ...
    %                     DoCOut = Data_DoCi(Data_DoCi.DoC<0.4,:);
                ...
                if display1 || ~printOutFig
                    ...
                    if length(Nb) > DBSCANParams.Cutoff % Does this switch do anything?
                        ...
                        plot(ax1, contour(:,1), contour(:,2), 'color', rgb(44, 62, 80)); % rgb(44, 62, 80) # roi区域外框的颜色,这里画的并不是边框，是散点图
                        %text(ax1, contour(:,1), contour(:,2), "1")
                        %text(100,100,'1')
                        %Z = 1;
                        %contour(contour(:,1), contour(:,2), Z, 'ShowText', 'on')
                        %[C,h] = contour();
                        %clabel(C,h)
                        %text(contour(1,1), contour(1,2), string(i), Color='red', HorizontalAlignment='right',FontSize=10)
    ...
    %         
        ...
        if DBSCANParams.DoStats
            % 应该是指代最终的result输出
            Result.Number_Cluster = numel(ClusterSmooth); % numel计算满足指定条件的个数    不指定条件的话，即为 n = numel(arry)，用于统计 arry 元素个数
            Result.Number(1) = mean(cell2mat(cellfun(@(x) x.Nb(x.Nb > DBSCANParams.Cutoff), ...
                                    ClusterSmooth, 'UniformOutput', false)));
            Result.Area(1) = mean(cell2mat(cellfun(@(x) x.Area(x.Nb > DBSCANParams.Cutoff), ...
                                    ClusterSmooth, 'UniformOutput', false)));
            Result.Mean_Circularity(1) = mean(cell2mat(cellfun(@(x) x.Circularity(x.Nb > DBSCANParams.Cutoff), ...
                                    ClusterSmooth, 'UniformOutput', false)));
            Result.Density = mean(cell2mat(cellfun(@(x) x.Density(x.Nb > DBSCANParams.Cutoff), ...
                                    ClusterSmooth, 'UniformOutput', false)));
