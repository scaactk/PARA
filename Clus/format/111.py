...
def doFormat(path, name):
    ...
    if flagRed == 1:  # red exist
        rawRed = pd.read_csv(os.path.join(path, "Red.3dlp"), skiprows=0, header=None, sep='\s+', usecols=[0, 1], engine='python')
...
def findAllFile(base, Range):  # 20220911
    ...
    for root, dirs, files in os.walk(base):
        for name in dirs:
            if name.isnumeric() and int(name) in Range:
                print(name)
...
#  keep the r before '***'
father_path = r"F:\20221214qw"  # now do not need to change '/' and '\' in path,
Range = input("Please input the ID of folder: ").split(" ")
